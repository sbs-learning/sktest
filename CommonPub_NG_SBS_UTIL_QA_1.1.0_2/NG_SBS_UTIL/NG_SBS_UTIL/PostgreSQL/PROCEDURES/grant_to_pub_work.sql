CREATE OR REPLACE PROCEDURE sbs_util.grant_to_pub_work
(
...
/* --------------------------------------------------------------------------
   -- Purpose : Purpose of this Procedure is to provide grant on pub_work schema.
   -- Parameter Details
   ...
   --------------------------------------------------------------------------------
   -- Name: grant_to_pub_work
   --------------------------------------------------------------------------------
   --
   -- Description:  grant_to_pub_work
   --
   --------------------------------------------------------------------------------
   -- RefNo Name            JIRA NO 		Date     Description of change
   -- ----- ---------------- -------- ---------------------------------------------
   -- 	1	Sakshi Jain	 CSPUBCC-4303	03/16/2021	 grant_to_pub_work() initial draft
...
DECLARE
  ...
  BEGIN
	...
	l_related_schema := sbs_util.get_pub_work_schema();
	...
	WHEN null_value_not_allowed THEN
		...
		CALL sbs_util.error_handler
			(
				...
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_pub_work(), Input in_privilege / in_object / in_object_type parameter is NULL or empty ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN SQLSTATE '50001' THEN
		...
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_pub_work(), Input in_grantable parameter is invalid it should be YES/NO ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN SQLSTATE '50002' THEN
		...
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_pub_work(), No related user found for pub_work ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN OTHERS THEN
		GET STACKED DIAGNOSTICS stack = PG_EXCEPTION_CONTEXT;
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_pub_work() ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
END;
$$;