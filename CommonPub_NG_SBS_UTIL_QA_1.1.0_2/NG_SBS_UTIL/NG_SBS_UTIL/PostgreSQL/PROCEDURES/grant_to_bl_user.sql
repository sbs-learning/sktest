CREATE OR REPLACE PROCEDURE sbs_util.grant_to_bl_user
(
...
/* --------------------------------------------------------------------------
   -- Purpose : Purpose of this Procedure is to provide grant on bl_user schema.
   -- Parameter Details
   ...
   --------------------------------------------------------------------------------
   -- Name: grant_to_bl_user
   --------------------------------------------------------------------------------
   --
   -- Description:  grant_to_bl_user
   --
   --------------------------------------------------------------------------------
   -- RefNo Name            JIRA NO 		Date     Description of change
   -- ----- ---------------- -------- ---------------------------------------------
   -- 	1	Sakshi Jain	 CSPUBCC-4303	03/16/2021	 grant_to_bl_user() initial draft
...
BEGIN
	...
	l_related_schema := sbs_util.get_bl_user_schema();
	...
	WHEN null_value_not_allowed THEN
		...
		CALL sbs_util.error_handler
			(
				...
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_bl_user(), Input in_privilege / in_object / in_object_type parameter is NULL or empty ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN SQLSTATE '50001' THEN
		...
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_bl_user(), Input in_grantable parameter is invalid it should be YES/NO ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN SQLSTATE '50002' THEN
		...
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_bl_user(), No related user found for bl_user ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
	WHEN OTHERS THEN
		GET STACKED DIAGNOSTICS stack = PG_EXCEPTION_CONTEXT;
		CALL sbs_util.error_handler
			(
				in_error_stack		=> stack,
				in_error_code		=> SQLSTATE,
				in_error_message	=> CONCAT(' Error in procedure :- grant_to_bl_user() ', CHR(10), ' SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
END;
$$;