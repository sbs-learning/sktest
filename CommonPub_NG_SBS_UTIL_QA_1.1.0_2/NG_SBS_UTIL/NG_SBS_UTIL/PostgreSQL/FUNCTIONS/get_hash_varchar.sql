CREATE OR REPLACE FUNCTION sbs_util.get_hash_varchar
(
...
DECLARE
 --------------------------------------------------------------------------
	-- Purpose : To calculate hash of given string.
    -- Parameter Details
	--in_string : String to convert
      -----------------------------------------------------------------------------------------------------------------------------
   --
   --------------------------------------------------------------------------------
   -- Name: get_hash_varchar
   --------------------------------------------------------------------------------
   --
   -- Description:  get_hash_varchar
   --
   --------------------------------------------------------------------------------
	...
	WHEN OTHERS THEN
		...
		CALL sbs_util.error_handler
			(
				...
				in_error_message	=> CONCAT(' Error in function :- get_hash_varchar(), ', CHR(10), 'SQLERRM: ', SQLERRM),
				in_show_stack		=> TRUE
			);
END;
$BODY$;